...
const INITIAL_FINDINGS: Finding[] = [
  {
    ...
    administrativeControls: "Capacitación",
    eppControls: "Casco y calzado",
  ...
  },
  {
    ...
    description: "Exposición a ruido continuo",
    ...
    administrativeControls: "Rotación",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    let label = "Bajo";
    if (np >= 24 && np <= 40) label = "Muy Alto";
    else if (np >= 10 && np <= 20) label = "Alto";
    else if (np >= 6 && np <= 8) label = "Medio";
    setCalculatedProbLabel(label);
    const nr = np * consequenceLevel;
    setCalculatedRisk(nr);
    let tier = "IV", acceptability = "Aceptable";
    if (nr >= 600) { tier = "I"; acceptability = "Inaceptable"; }
    else if (nr >= 120) { tier = "II"; acceptability = "Requiere control"; }
    else if (nr >= 50) { tier = "III"; acceptability = "Aceptable"; }
  ...
  const handleSaveFinding = (e: React.FormEvent) => {
    ...
    if (!editingFinding && findings.length >= 30) {
      alert("Límite máximo de 30 hallazgos");
      return;
    }
  ...
  const stats = { total: findings.length, abiertos: findings.filter(f => f.status === "Abierto").length, criticos: findings.filter(f => f.riskTier === "I" || f.riskTier === "II").length };
  const percentCerrados = stats.total > 0 ? Math.round((findings.filter(f => f.status === "Cerrado").length / stats.total) * 100) : 0;

  return (
    <div className="min-h-screen bg-slate-50">
      <header className="bg-indigo-900 text-white p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2"><ClipboardCheck className="w-6 h-6" /><h1 className="font-bold">Matriz de Riesgos SST GTC 45</h1></div>
          <div><button onClick={exportToExcelGTC45} className="bg-emerald-600 px-3 py-1 rounded mr-2">📊 Exportar</button><button onClick={handleNewFindingClick} className="bg-white text-indigo-900 px-3 py-1 rounded">➕ Nuevo</button></div>
        </div>
      </header>
      <main className="max-w-7xl mx-auto p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-gradient-to-r from-indigo-800 to-indigo-950 text-white p-4 rounded-xl"><span className="text-2xl font-bold">{stats.total}</span><p>Total Hallazgos</p><div className="mt-2 bg-indigo-950 rounded-full h-2"><div className="bg-emerald-400 rounded-full h-2" style={{ width: `${percentCerrados}%` }}></div></div></div>
          <div className="bg-white p-4 rounded-xl shadow"><span className="text-2xl font-bold text-rose-600">{stats.criticos}</span><p>Críticos</p></div>
          <div className="bg-white p-4 rounded-xl shadow"><span className="text-2xl font-bold text-amber-600">{stats.abiertos}</span><p>Abiertos</p></div>
          <div className="bg-white p-4 rounded-xl shadow"><span className="text-2xl font-bold text-emerald-600">{findings.filter(f => f.status === "Cerrado").length}</span><p>Cerrados</p></div>
        </div>
        <div className="bg-white p-4 rounded-xl shadow mb-4"><Search className="inline mr-2 w-4 h-4" /><input type="text" placeholder="Buscar..." className="w-full p-2 border rounded" onChange={e => setSearchTerm(e.target.value)} /></div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            {findings.filter(f => {
              const text = `${f.location} ${f.technicalObservation}`.toLowerCase();
              return text.includes(searchTerm.toLowerCase()) && (factorFilter === "Todos" || f.riskFactor === factorFilter);
            }).map(f => (
              <div key={f.id} className="bg-white rounded-xl shadow p-4">
                <div className="flex justify-between"><div><h3 className="font-bold">{f.location}</h3><p className="text-sm text-indigo-600">{f.riskFactor}</p><p className="text-sm mt-1">{f.description}</p></div><div><button onClick={() => handleEditClick(f)} className="text-blue-600 mr-2">✏️</button><button onClick={() => handleDeleteClick(f.id)} className="text-red-600">🗑️</button></div></div>
                <div className="grid grid-cols-4 gap-2 mt-3 text-center text-xs"><div>ND: {f.deficiencyLevel}</div><div>NE: {f.exposureLevel}</div><div>NP: {f.probabilityLevel}</div><div>NR: {f.riskLevelValue}</div></div>
          ...
          <div className="space-y-4">
            <div className="bg-white p-4 rounded-xl shadow">
              <div className="flex items-center gap-2 text-indigo-600"><Sparkles className="w-5 h-5" /><h4 className="font-bold">Asistente IA GTC 45</h4></div>
              <textarea className="w-full p-2 border rounded mt-2 text-sm" rows={3} placeholder="Describe el hallazgo..." value={technicalObservation} onChange={e => setTechnicalObservation(e.target.value)}></textarea>
              <button onClick={() => setIsAICopierOpen(true)} className="w-full bg-indigo-600 text-white py-2 rounded mt-2">🤖 Obtener Prompt IA</button>
            </div>
          </div>
        </div>
      </main>
      {isFormOpen && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4"><div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto"><h3 className="font-bold text-lg mb-4">{editingFinding ? "Editar" : "Nuevo"} Hallazgo</h3><form onSubmit={handleSaveFinding} className="space-y-3"><input className="w-full p-2 border rounded" placeholder="Ubicación" value={location} onChange={e => setLocation(e.target.value)} required /><textarea className="w-full p-2 border rounded" rows={3} placeholder="Observación técnica" value={technicalObservation} onChange={e => setTechnicalObservation(e.target.value)} required /><div className="border-2 border-dashed border-slate-200 rounded-lg p-4 text-center cursor-pointer" onClick={() => fileInputRef.current?.click()}><input type="file" ref={fileInputRef} onChange={handleImageUploaded} accept="image/*" className="hidden" />{imageUrl ? <img src={imageUrl} className="max-h-32 mx-auto" /> : "📸 Haga clic para subir imagen"}</div><select className="w-full p-2 border rounded" value={riskFactor} onChange={e => handleRiskFactorChange(e.target.value)}>{Object.keys(RISK_FACTORS_MAP).map(f => <option key={f}>{f}</option>)}</select><div className="grid grid-cols-3 gap-2"><select className="p-2 border rounded" value={deficiencyLevel} onChange={e => setDeficiencyLevel(Number(e.target.value))}><option value={10}>ND: Muy Alto</option><option value={6}>ND: Alto</option><option value={2}>ND: Medio</option></select><select className="p-2 border rounded" value={exposureLevel} onChange={e => setExposureLevel(Number(e.target.value))}><option value={4}>NE: Continua</option><option value={3}>NE: Frecuente</option><option value={2}>NE: Ocasional</option></select><select className="p-2 border rounded" value={consequenceLevel} onChange={e => setConsequenceLevel(Number(e.target.value))}><option value={100}>NC: Mortal</option><option value={60}>NC: Muy Grave</option><option value={25}>NC: Grave</option></select></div><input className="w-full p-2 border rounded" placeholder="Plan de acción" value={actionPlan} onChange={e => setActionPlan(e.target.value)} /><button type="submit" className="w-full bg-indigo-600 text-white py-2 rounded">Guardar</button><button type="button" onClick={() => setIsFormOpen(false)} className="w-full border py-2 rounded">Cancelar</button></form></div></div>}
      {isAICopierOpen && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4"><div className="bg-white rounded-xl p-6 max-w-2xl w-full"><h4 className="font-bold">Asistente IA</h4><textarea className="w-full p-2 border rounded mt-2" rows={6} placeholder="Pega la respuesta de la IA aquí..." value={externalAIInput} onChange={e => setExternalAIInput(e.target.value)}></textarea><div className="flex gap-2 mt-4"><button onClick={handleCopyAIPrompt} className="flex-1 bg-indigo-600 text-white py-2 rounded">📋 Copiar Prompt</button><button onClick={handleApplyExternalAI} className="flex-1 bg-emerald-600 text-white py-2 rounded">🔄 Aplicar Análisis</button><button onClick={() => setIsAICopierOpen(false)} className="flex-1 border py-2 rounded">Cancelar</button></div></div></div>}
    </div>
  );
}
